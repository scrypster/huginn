          ...
          <template v-if="currentProvider === 'ollama'">
            ...
            <section class="space-y-4">
              <div class="flex items-center justify-between">
                <h3 class="text-[11px] font-semibold text-huginn-muted uppercase tracking-widest">Installed Models</h3>
                <button @click="loadAvailableModels" class="text-xs text-huginn-muted hover:text-huginn-blue transition-colors">Refresh</button>
              </div>
              ...
              <div class="flex items-center gap-2">
                <input
                  ...
                  class="flex-1 bg-huginn-surface border border-huginn-border rounded-lg px-3 py-2 text-sm text-huginn-text font-mono outline-none focus:border-huginn-blue/50 transition-colors"
                />
                <button
                  @click="pullModel(pullModelName)"
                  :disabled="!pullModelName || pulling"
                  class="px-3 py-2 rounded-lg text-xs font-medium border transition-all duration-150 disabled:opacity-50"
                  :class="pulling
              ...
              <!-- Pull feedback -->
              <div v-if="pullMsg"
                class="px-4 py-2.5 rounded-xl border text-xs"
                :class="pullError
              ...
              <div v-if="deleteError"
                class="flex items-center justify-between gap-2 px-4 py-2 rounded-xl border border-huginn-red/30 bg-huginn-red/8 text-huginn-red text-xs">
                <span>{{ deleteError }}</span>
                <button @click="deleteError = null" class="opacity-60 hover:opacity-100">✕</button>
              </div>

              <div v-if="modelsLoading" class="flex items-center gap-2 text-huginn-muted text-xs py-4">
                <div class="w-3.5 h-3.5 border border-huginn-muted border-t-huginn-blue rounded-full animate-spin" />
                Checking Ollama...
              </div>

              <div v-else-if="ollamaStatus === 'error'" class="py-6 text-center space-y-1">
              ...
              <div v-else-if="availableModels.length === 0" class="py-6 text-center">
                ...
                <p class="text-[11px] text-huginn-muted/60">Pull a model using the input above</p>
              </div>

              <div v-else class="space-y-2">
                <div v-for="m in availableModels" :key="m.name"
                  class="flex items-center gap-3 px-3 py-2.5 rounded-xl border border-huginn-border bg-huginn-surface/50 group">
                  <div class="w-7 h-7 rounded-lg flex items-center justify-center flex-shrink-0"
                    style="background:rgba(88,166,255,0.08);border:1px solid rgba(88,166,255,0.15)">
                    <svg class="w-3.5 h-3.5 text-huginn-blue/70" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round">
                      <path d="M12 2L2 7l10 5 10-5-10-5z" /><path d="M2 17l10 5 10-5" /><path d="M2 12l10 5 10-5" />
                    </svg>
                  </div>
                  <div class="flex-1 min-w-0">
                    <p class="text-xs font-medium text-huginn-text font-mono truncate">{{ m.name }}</p>
                    <p v-if="m.details?.parameter_size" class="text-[11px] text-huginn-muted">
                      {{ m.details.parameter_size }}
                      <span v-if="m.details.quantization_level"> · {{ m.details.quantization_level }}</span>
                  ...
                  <div class="flex items-center gap-2 flex-wrap">
                    <span
                      ...
                      :key="agentName"
                      class="text-[10px] px-1.5 py-0.5 rounded border border-huginn-blue/30 text-huginn-blue">
                      {{ agentName }}
                    </span>
                    <button
                      @click="deleteOllamaModel(m.name)"
                      :disabled="deletingModels.has(m.name)"
                      class="opacity-0 group-hover:opacity-100 transition-opacity text-[10px] px-1.5 py-0.5 rounded border border-huginn-red/30 text-huginn-red hover:bg-huginn-red/10 disabled:opacity-50"
                      title="Remove model">
                      {{ deletingModels.has(m.name) ? '…' : 'delete' }}
                    </button>
                  </div>
                </div>
              </div>
