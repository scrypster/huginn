...
### Fixed
- `wait_for_threads` called with no `thread_ids` (model-omitted or auto-wait) now also checks recently spawned thread IDs, not just `ActiveThreadIDs`: a fast specialist that reaches `StatusDone` before wait runs is no longer invisible to an empty-args wait, so the lead gets the real result instead of "No matching threads" and an invented answer
