...
### Added
- Perf wave step 2 (prompt budget + ollama keep-alive + streaming first-signal), measured against the step-1 turn-latency telemetry: a trivial-classified turn that doesn't need the roster as an answer source (`agent.IsTrivialAsk` minus who-is-here/headcount) now gets a skeleton system prompt — identity line + personality addendum only, skipping the team roster, available-models block, capability addendum, and cross-session memory (measured ~2.1KB → ~16B on a 7-agent roster; real ollama TTFT ~406ms → ~166ms for the same-shaped prompt, uncached). Non-trivial turns and every delegated worker/coding run are byte-for-byte unchanged. The trivial hallway (`completeTrivialAsk`) skips `RunLoop` entirely but still stamps `t_request`/`t_first_token`/`t_complete` and writes a `turn_metrics` row (`turn_kind="trivial"`) the same way `RunLoop` does, so this win is visible in `GET /api/v1/metrics/turns` and the `turn_metrics` log line like every other turn kind, not just claimed in this changelog entry. A genuinely-ollama chat request now sends ollama's `keep_alive` field (default `10m`, configurable via `backend.keep_alive`) so a warm model stays resident between turns — measured ~4.7s cold reload vs ~56ms warm TTFT on qwen2.5-coder:14b; openai/deepseek/zai/custom/anthropic/vertex/openrouter/managed/the generic `external` provider are all untouched (`keep_alive` is opt-in per construction site, not a default `ExternalBackend` carries — `NewExternalBackendWithAPIKey` and the bare `NewExternalBackend` constructor never set it). Serve start fires one best-effort, non-blocking warm-up request for the default agent's model and the Chief of Staff's model (bounded to those two) so the first real turn doesn't pay a cold load. Streaming audit: no fixed batching window was found at the SSE/WS layers (both flush every delta immediately) — the one real "silent JSON-parse phase" gap was a straight-to-tool-call turn with no prose, where the user previously saw nothing between "thinking" and the `tool_call` event RunLoop only fires once the full response has streamed; both the content-embedded and native `delta.tool_calls` tool-call paths now emit one `StreamStatus{"using tools"}` (existing event type, never persisted) the moment the stream commits to a tool call.
- Turn-latency telemetry (perf-wave foundation): every agent turn through `RunLoop` stamps `t_request` (loop entry), `t_first_token`/`t_first_signal` (first streamed content or tool-start, whichever comes first), and `t_complete`, plus model/provider/agent/session/turn-kind/prompt-size/tool-call-count. Persisted async to a new bounded `turn_metrics` SQLite table (single writer goroutine, non-blocking `Enqueue` with a drop counter on overflow, pruned to the last 10k rows) — the hot turn path never does blocking I/O. `GET /api/v1/metrics/turns` (authed) returns recent turns plus a per-model p50/p95 summary for first-token and complete latency; a greppable `turn_metrics` INFO log line (`model=… agent=… ft_ms=… complete_ms=…`) fires per turn. Metadata only — no token or message content is ever recorded.
- User-configurable hooks: `.huginn/hooks.json` (workspace) and `~/.huginn/hooks.json` (global) register shell-command `PreToolUse`/`PostToolUse` hooks into the same internal chain G1 syntax validation runs on. A `PreToolUse` hook that exits non-zero vetoes the tool call (stdout/stderr become the deny reason shown to the model); `PostToolUse` is observe-only. Off by default — no hooks.json anywhere means no user hooks run. Hooks are matched by exact tool name or glob (`"*"` = all), reload automatically on file change (and on demand via `POST /api/v1/hooks/reload`), and every real execution is recorded in a per-runner audit log (`GET /api/v1/hooks/audit`). New Settings → Hooks page lists/adds/edits/toggles/deletes hooks and can dry-run one against a sample tool call without touching hooks.json; dry-runs ARE recorded in the audit log (tagged `test_run: true`) so the trail stays complete while staying distinguishable from real executions; a "copy schema example" button hands a model the exact JSON shape so it can author `hooks.json` itself via its own file tools (gated by the normal write-permission confirmation, no separate create_hook tool).
- PR-as-deliverable, closed end to end: `gh_pr_create`/`glab_mr_create` return the new PR/MR's URL (and number) in `ToolResult.Metadata`, not just a trimmed CLI stdout dump; their tool description hints the default base/target branch (resolved once from `origin/HEAD` at registration) so the model doesn't have to guess or spend a tool call finding it. Hallway/thread coding-run bubbles render a **PR card** (clickable URL, PR number/title, pushed branch, checks pill from `gh_pr_checks`/`glab_mr_checks`) alongside the DiffCard, and a run that both changed files and opened a PR gets a deliverable-handoff completion line ("Opened PR #123: \<title\> — 2 files changed (+8 −3). Checks: pending.") instead of the plain changed-files line — all derived UI-side from persisted tool calls, no engine text injection.
...
- Edit-time syntax validation (G1) now covers JavaScript (`node --check`, esbuild fallback), TypeScript/TSX/JSX (esbuild parse), PHP (`php -l`), Ruby (`ruby -c`), and Rust (`rustfmt --check`), in addition to Go and Python. Each validator is `LookPath`-gated and skips honestly (reports unchecked, never a false syntax failure) when its tool isn't installed. Rust distinguishes real parse errors (`error:` on stderr) from formatting-only diffs so valid-but-unformatted code is not blocked; TypeScript validation parses only, so a type error is not reported as a syntax failure. LSP server detection extended for Ruby (ruby-lsp/solargraph) and PHP (intelephense/phpactor).
### Changed
...
### Fixed
- `find_definition`/`list_symbols` now route by file extension to the matching language server instead of collapsing every detected server onto one (which sent, e.g., TypeScript lookups in a Go repo to `gopls` and returned wrong results); when no server matches a file's language they return a clean "no language server configured" message rather than results from the wrong server.
