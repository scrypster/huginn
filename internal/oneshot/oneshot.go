...
func Run(ctx context.Context, cfg Config) (*Result, error) {
	...
	// Oneshot collects OnToken into AgentOutput. The token gate hides
	// leading tool JSON, but after a permission deny the model often
	// writes another harness object into content — strip that too so
	// {"name":"gh_issue_create",...} never appears in the answer.
	denied := false
	for _, c := range result.ToolsCalled {
