...
func TestRun_DeniedTool_HidesLeftoverHarnessJSON(t *testing.T) {
	...
	res, err := Run(context.Background(), Config{
		...
		AgentName: "Steve",
		// CLI without --dangerously-skip-permissions: exec is denied.
...
func TestRun_DeniedTool_PureIssueJSONHidden(t *testing.T) {
	...
	if strings.Contains(res.AgentOutput, "gh_issue_create") || strings.Contains(res.AgentOutput, leftoverJSONNeedle(issueJSON)) {
		t.Errorf("pure leftover JSON leaked into agentOutput: %q", res.AgentOutput)
	}
}

func leftoverJSONNeedle(s string) string {
	if strings.Contains(s, "gh_issue_create") {
