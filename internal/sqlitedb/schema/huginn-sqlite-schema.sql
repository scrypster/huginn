...
-- turn_metrics: per-agent-turn latency telemetry (t_request/t_first_token/
-- t_complete + dimensions), async-drained from internal/turnmetrics.Writer.
-- Pruned to the last 10 000 rows by the writer. Metadata only — never token
-- or message content. Backs GET /api/v1/metrics/turns.
CREATE TABLE IF NOT EXISTS turn_metrics (
    ...
    first_token_ms    INTEGER NOT NULL DEFAULT -1,   -- -1 = no token streamed (tool-only/error turn)
